...
static void I_StartupConsole(void)
{
	...
	if (gotConsole)
	{
		SetConsoleTitleA("SRB2P Console");
...
void I_UpdateMumble(const mobj_t *mobj, const listener_t listener)
{
	...
	if(mumble->uiVersion != 2) {
		wcsncpy(mumble->name, L"SRB2P "VERSIONSTRINGW, 256);
		wcsncpy(mumble->description, L"Sonic Robo Blast 2 Persona with integrated Mumble Link support.", 2048);
		mumble->uiVersion = 2;
	}
	mumble->uiTick++;
...
INT32 I_StartupSystem(void)
{
	...
	if (SDL_Init(0) < 0)
		I_Error("SRB2P: SDL System Error: %s", SDL_GetError()); //Alam: Oh no....
#ifndef NOMUMBLE
	I_SetupMumble();
#endif
	return 0;
...
void I_Error(const char *error, ...)
{
	...
	if (shutdowning)
	{
		...
		if (errorcount > 20)
		{
			...
			if (!M_CheckParm("-dedicated"))
				SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR,
					"SRB2P "VERSIONSTRING" Recursive Error",
					buffer, NULL);
